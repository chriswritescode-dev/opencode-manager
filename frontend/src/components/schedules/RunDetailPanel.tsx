...
export function RunDetailPanel({ repoId, activeRun, selectedRunLoading, onCancelRun, cancelRunPending }: RunDetailPanelProps) {
  ...
  if (!activeRun) {
    return <div className="flex h-full items-center justify-center text-sm text-muted-foreground">{t('schedule.selectRunHint')}</div>
  }
    ...
    <Tabs key={`${activeRun.id}-${String(activeRun.responseText ? 'response' : activeRun.errorText ? 'error' : 'log')}`} defaultValue={activeRun.responseText ? 'response' : activeRun.errorText ? 'error' : 'log'} className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <div className="flex items-center justify-center gap-2 px-2 py-1.5">
        <TabsList className="h-auto gap-0 rounded-none border-0 bg-transparent p-0">
          <TabsTrigger value="log" className="rounded-none border-b-2 border-transparent px-3 py-1.5 text-xs data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none">{t('schedule.log')}</TabsTrigger>
          <TabsTrigger value="response" disabled={!activeRun.responseText} className="rounded-none border-b-2 border-transparent px-3 py-1.5 text-xs data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none">{t('assistant.output')}</TabsTrigger>
          <TabsTrigger value="error" disabled={!activeRun.errorText} className="rounded-none border-b-2 border-transparent px-3 py-1.5 text-xs data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none">{activeRun.status === 'cancelled' ? t('schedule.details') : t('common.error')}</TabsTrigger>
        </TabsList>
      </div>
      ...
      <TabsContent value="log" className="mt-0 min-h-0 flex-1 overflow-y-auto px-0 py-3 [mask-image:linear-gradient(to_bottom,transparent,black_16px,black)]">
        {selectedRunLoading && !activeRun ? (
          ...
          <pre className="whitespace-pre-wrap break-words text-sm font-mono leading-6">{activeRun.logText ?? t('schedule.noLogText')}</pre>
        )}
      </TabsContent>
      <TabsContent value="response" className="mt-0 min-h-0 flex-1 flex flex-col overflow-hidden">
        {selectedRunLoading && !activeRun ? (
          ...
          <div className="p-3"><pre className="whitespace-pre-wrap break-words text-sm font-mono leading-6">{t('schedule.noAssistantOutput')}</pre></div>
        )}
      </TabsContent>
      <TabsContent value="error" className="mt-0 min-h-0 flex-1 overflow-y-auto px-0 py-3 [mask-image:linear-gradient(to_bottom,transparent,black_16px,black)]">
        {selectedRunLoading && !activeRun ? (
          ...
          <pre className={`whitespace-pre-wrap break-words text-sm font-mono leading-6 ${activeRun.status === 'cancelled' ? 'text-muted-foreground' : 'text-red-300'}`}>{activeRun.errorText ?? t('schedule.noErrorRecorded')}</pre>
        )}
      </TabsContent>
    </Tabs>
  )
