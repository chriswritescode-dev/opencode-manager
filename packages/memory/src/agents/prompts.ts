type AgentRole = 'code' | 'auditor' | 'architect'
...
const SCOPE_DESCRIPTIONS: Record<AgentRole, string> = {
...
const SCOPE_GUIDANCE: Record<AgentRole, string> = {
...
If you notice patterns of outdated or incorrect memories, consider asking the user to curate them. Use the @Librarian subagent to perform memory research and contradiction resolution.`,
  auditor: `These memories may be stale or irrelevant. If a memory seems outdated, note it in your review observations.`,
  architect: `These memories may be stale or irrelevant. Use your judgement — if a memory seems outdated, note it in your plan and recommend updating or deleting it via memory-edit or memory-delete.`,
}

export function getInjectedMemory(role: AgentRole): string {
